...
public class PreconditionsTest extends TestCase {
    ...
    public void testAllOverloads_checkNotNull() throws Exception {
        for (ImmutableList<Class<?>> sig : allSignatures(Object.class)) {
            ...
            checkArgumentMethod.invoke(null /* static method */, getParametersForSignature(new Object(), sig));
    ...
    private static final ImmutableList<Class<?>> possibleParamTypes = ImmutableList.of(char.class, int.class, long.class,
                                                                                       Object.class);
    ...
    private static ImmutableList<ImmutableList<Class<?>>> allSignatures(Class<?> predicateType) {
        ...
        for (int i = 0; i < 2; i++) {
            ...
            for (List<Class<?>> curr : Lists.cartesianProduct(typesLists)) {
                allOverloads.add(ImmutableList.<Class<?>> builder().add(predicateType).add(String.class) // the
                                                                                                         // format
                                                                                                         // string
                                .addAll(curr).build());
            }
    ...
    public void overloadSelection() {
        ...
        // ambiguous without the cast to Object because the boxed predicate prevents any overload
        // from
        ...
        // operation) no overload can be selected in phase 1. and in phase 2 it is ambiguous since
        // it
        // matches with the second parameter being boxed and without it being boxed. The cast to
        // Object
        // avoids this.
        checkState(aBoolean, "", aBoolean ? "" : anInt, (Object) anInt);

        // ambiguous without the .booleanValue() call since the boxing forces us into phase 2
        // resolution
        short s = 2;
